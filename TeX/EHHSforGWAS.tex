...
In order to quantify the effect of the neighbouring homozygosity at a given studied position ($i$),  all values for $EHHS_{ij}$ are summed from  the position $i$,to their right and and their left sides, all the contributions of $EHHS_{ij}$, considering their distance (physical or recombinant), and until a threshold arbitrary value of $EHHS_{ij}$ (let's say 0.1). That is: 

\begin{equation}
 iES_{d_i} = \sum_{j=x+1}^{y}\frac{(EHHS_{d_{ij-1}} + EHH_{d_{ij})}}{2} (Pos_{j} - Pos_{j-1}),
 \end{equation}
...
The integrated relative Extended homozygosity statistic is then calculated as:
 \begin{equation}
 iES_{da_{i}} = \frac{iES_{d_{i}}} {iES_{a_{i}}}.
 \end{equation}
